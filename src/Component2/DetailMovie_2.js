...
class DetailMovie_2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            idListTheaterMovie: [],
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.detailMovie) {
            if (nextProps.detailMovie.lichChieu) {
                let idTheater = nextProps.detailMovie.lichChieu.map((item) => {
                    return item.thongTinRap.maHeThongRap
                })
                let arrFilter = idTheater.filter((item, index) => {
                    idTheater.indexOf(item) === index
                })
                let arr = []
    ...
    handleOnChangeId = (idTheater) => {
        if (this.props.detailMovie) {
            let arrTheater = this.props.detailMovie.filter((item) => {
                return item.thongTinRap.maHeThongRap === idTheater
            })
            this.setState({
                theater: arrTheater[0],
    ...
    renderLogo = () => {
        if (this.state.listTheaterMovie) {
            return this.state.listTheaterMovie.map((item, index) => {
                return (
                    <a
                        onClick={() => {
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(DetailMovie_2));